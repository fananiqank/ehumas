...
$('.server-side').DataTable( {
        ...
        "ajax": "apps/trijinvisa2/data.php"
    } );
    ...
    $('#form input,#form select, #form select2 , #form textarea').jqBootstrapValidation({
        ...
        submitSuccess: function($form, event){     
            ...
            if($('#jumisi').val()==$('#jumdata').val()){
                ...
                $.post('apps/trijinvisa2/proses.php?act=post',data,
                    function(msg) {
...
//       {
//         alert(data);
//          window.location='index.php?x=trijinvisa2';
...
function delCart(a){
    if($('idmtc').val() == ''){
        $.get( "apps/trijinvisa2/proses.php?act=del&id="+a, function( data ) {
            // $( ".result" ).html( data );
...
function delCart2(a){
        $.get( "apps/trijinvisa2/proses2.php?act=del&id="+a, function( data ) {
            // $( ".result" ).html( data );
...
$(document).on('click','#detailrh',function(e){
    e.preventDefault();
        ...
        $.post('apps/trijinvisa2/detailrh.php?id='+$(this).attr("data-id"),
                function(html){
                $("#tampilhis").html(html);
                }   
            );
...
function uploadFile(urut,id) {
   ...
   if(files.length > 0 ){
      ...
      xhttp.open("POST", "apps/trijinvisa2/upload.php", true);
      ...
      xhttp.onreadystatechange = function() {
         if (this.readyState == 4 && this.status == 200) {
           ...
           if(response == 1){
              alert("UPLOAD SUKSES");
              $('#file_'+urut).val('');
              $('#syaratupload').load("apps/trijinvisa2/isiupload.php?rel=1");
           } else if(response == 2){
