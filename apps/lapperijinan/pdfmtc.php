...
foreach($db->select("tx_perijinan a join m_ijinjenis b on a.ijinjenis_id = b.ijinjenis_id","*","a.ijin_kode='$_GET[kode]' ") as $tipe){}
$jenisIjin = $tipe['ijinjenis_id'];
$ID = $tipe['ijin_id'];
//echo $jenisIjin;

if($jenisIjin == 7 || $jenisIjin == 8) {
foreach($db->select("tx_perijinan a join m_ijinjenis b on a.ijinjenis_id=b.ijinjenis_id left join tx_kalibrasi c on a.ijin_id=c.ijin_id","a.*,b.ijinjenis_name,c.*, case when c.kalbar_jenisijin=1 then 'Baru' when c.kalbar_jenisijin=2 then 'Perpanjangan' else 'Perbaikan' end as jeniskalbar","a.ijin_id = '$ID'") as $val2){} 
}
else if($jenisIjin == 9) {
foreach($db->select("tx_perijinan a join m_ijinjenis b on a.ijinjenis_id=b.ijinjenis_id left join tx_imb c on a.ijin_id=c.ijin_id","a.*,b.ijinjenis_name,c.*, case when c.imb_type=1 then 'Baru' when c.imb_type=2 then 'Lama' when c.imb_type=3 then 'Penambahan' when c.imb_type=4 then 'Perbaikan' else 'Renovasi' end as jenisimb","a.ijin_id = '$ID'") as $val2){} 
}
else{
foreach($db->select("tx_perijinan a join m_ijinjenis b on a.ijinjenis_id=b.ijinjenis_id join m_dep c on a.dep_id=c.id_dep left join m_jabatan e on a.id_jabatan=e.id_jabatan left join tx_bvisa d on d.ijin_id = a.ijin_id ","a.*,b.ijinjenis_name,b.skema_id,c.nama_dep,DATEDIFF(a.ijin_tglakhirterbit,a.ijin_tglawalterbit) as masaberlaku,e.nama_jabatan, case when d.bvisa_jenis=1 then '211a/211B Single-Entry' else 'Multiple-Entry (VKUBP)' end as jenisvisa","a.ijin_id = '$ID'") as $val2){} 
}
