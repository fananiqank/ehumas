...
$columns = array(
	...
	array('db'      => 'tgltransinput','dt'   => 4, 'field' => 'tgltransinput',
		   'formatter' => function( $d, $row ) {
	...
	array('db'      => 'pekerjaan','dt'   => 5, 'field' => 'pekerjaan',
		   'formatter' => function( $d, $row ) {
	...
	array('db'      => 'biaya','dt'   => 6, 'field' => 'biaya',
		   'formatter' => function( $d, $row ) {
...
$joinQuery = "FROM (select @rownum:=@rownum+1 no_urut,a.*,b.name_mekanik,c.no_mtc,concat(substr(d.arm_norangka,-5),' - ',d.arm_nolambung) as gabs from tx_mekanik a join m_mekanik b using(id_mekanik) join tx_maintenance c using(id_mtc) join m_armada d using(arm_id) JOIN (SELECT @rownum:=0) r where DATE(tgltransinput) between '$_GET[tgl1]' and '$_GET[tgl2]' $idmekanik $armada) a ";
$extraWhere = "";        
